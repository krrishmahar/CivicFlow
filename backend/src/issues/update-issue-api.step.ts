...
import { getDistanceFromLatLonInMeters, calculateCentroid } from '../utils/geo.utils';
  ...
  try {
    ...
    for (const issue of issues) {
      ...
      if (nearbyIssues.length + 1 >= minIssues) {
        ...
        // Calculate centroid using utility function
        const centroid = calculateCentroid(
          clusterIssues.map((i) => ({ lat: i.latitude, lng: i.longitude }))
        );
