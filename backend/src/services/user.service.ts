...
export const createUser = async (
  ...
  const existingUserId = await state.get<string | null>(USER_EMAILS_GROUP, normalizedEmail)
  if (existingUserId) {
    logger.warn('auth.signup.failed', { email: normalizedEmail, reason: 'EMAIL_ALREADY_EXISTS' })
    throw new BaseError('Email already in use', 409, 'EMAIL_ALREADY_EXISTS', { email: normalizedEmail })
  }
